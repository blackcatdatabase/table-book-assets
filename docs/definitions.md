...
| encryption_key_enc | mysql: BLOB / postgres: BYTEA | YES |  | Wrapped DEK or encrypted key blob. | `encrypt`<br/>ctx: `db.vault.book_assets.encryption_key_enc`<br/>kv: `key_version`<br/>meta: `encryption_meta` |
| encryption_iv | mysql: VARBINARY(32) | YES |  | IV/nonce used for encryption. |  |
...
| encryption_meta | mysql: JSON / postgres: JSONB | YES |  | JSON metadata about encryption layers. | meta for: `encryption_key_enc` |
| key_version | VARCHAR(64) | YES |  | Local key version reference. | key version for: `encryption_key_enc` |
