...
<div style='font-size:13px;opacity:0.92;'>Lineage heat: 3 outbound / 1 inbound · ✅ No engine drift detected · Index coverage: ready · PII: 4 · Changelog: fresh (2 d)</div>
</div>
...
- Pkg folder: [packages\book-assets](https://github.com/blackcatacademy/blackcat-database/blob/main/packages\book-assets)
...
graph LR
  ...
  book_assets["book_assets"]:::center
  book_assets -->|FK| books["books"]:::link
  book_assets -->|FK| crypto_keys["crypto_keys"]:::accent
  book_assets -->|FK| tenants["tenants"]:::link
  order_item_downloads["order_item_downloads"]:::inbound -->|FK| book_assets
...
- Outbound (depends on): "books", "crypto_keys", "tenants"
- Inbound (relies on this): "order_item_downloads"
...
- encryption_key_enc (key)
- id (key)
- key_id (key)
- key_version (key)

## Usage
- Install/upgrade schema: pwsh -NoLogo -NoProfile -File scripts/schema-tools/Migrate-DryRun.ps1 -Package book-assets -Apply
