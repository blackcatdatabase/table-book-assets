...
namespace BlackCat\Database\Packages\BookAssets\Repository;

use BlackCat\Core\Database as Database;              // e.g. BlackCat\Core\Database
use BlackCat\Database\Packages\BookAssets\Definitions;
use BlackCat\Database\Packages\BookAssets\Criteria;
use BlackCat\Database\Packages\BookAssets\Dto\BookAssetDto as Dto;
use BlackCat\Database\Packages\BookAssets\Mapper\BookAssetDtoMapper as RowMapper;
...
final class BookAssetRepository implements RepoContract, KeysetRepoContract
{
     ...
     */
    protected function def(): string { return \BlackCat\Database\Packages\BookAssets\Definitions::class; }
    ...
    private function doUpsert(array $row, bool $revive): void
    {
        ...
        $keys = [ 'tenant_id', 'book_id', 'asset_type' ];
        if (!$keys) {
    ...
    public function upsertMany(array $rows): int {
        ...
        if (class_exists(\BlackCat\Database\BulkUpsertRepository::class) && [ 'tenant_id', 'book_id', 'asset_type' ] !== []) {
            $bulk = new \BlackCat\Database\BulkUpsertRepository($this->db, \BlackCat\Database\Packages\BookAssets\Definitions::table());
            return $bulk->upsertMany($rows, [ 'tenant_id', 'book_id', 'asset_type' ], [ 'filename', 'mime_type', 'size_bytes', 'storage_path', 'content_hash', 'download_filename', 'is_encrypted', 'encryption_algo', 'encryption_key_enc', 'encryption_iv', 'encryption_tag', 'encryption_aad', 'encryption_meta', 'key_version', 'key_id' ]);
        }
    ...
    */
    ...
    /** @return array<string,mixed>|\BlackCat\Database\Packages\BookAssets\Dto\BookAssetDto|null */
    public function getByTenantIdAndBookIdAndAssetType(int $tenantId, int $bookId, string $assetType, bool $asDto = false): array|\BlackCat\Database\Packages\BookAssets\Dto\BookAssetDto|null {
        $row = $this->getByUnique([ 'tenant_id' => $tenantId, 'book_id' => $bookId, 'asset_type' => $assetType ]);
        if (!$asDto || !$row) return $row;
        return \BlackCat\Database\Packages\BookAssets\Mapper\BookAssetDtoMapper::fromRow($row);
    }
    public function existsByTenantIdAndBookIdAndAssetType(int $tenantId, int $bookId, string $assetType): bool {
        $where = 't.' . Ident::q($this->db, 'tenant_id') . ' = :uniq_tenant_id' . ' AND ' . 't.' . Ident::q($this->db, 'book_id') . ' = :uniq_book_id' . ' AND ' . 't.' . Ident::q($this->db, 'asset_type') . ' = :uniq_asset_type';
        return $this->exists($where, [ 'uniq_tenant_id' => $tenantId, 'uniq_book_id' => $bookId, 'uniq_asset_type' => $assetType ]);
    }
    /** @return int|string|null */
    public function getIdByTenantIdAndBookIdAndAssetType(int $tenantId, int $bookId, string $assetType) {
        $row = $this->getByTenantIdAndBookIdAndAssetType($tenantId, $bookId, $assetType);
        return $row ? ($row['id'] ?? null) : null;
    }
    /** @return array<string,mixed>|\BlackCat\Database\Packages\BookAssets\Dto\BookAssetDto|null */
    public function getByTenantIdAndId(int $tenantId, int $id, bool $asDto = false): array|\BlackCat\Database\Packages\BookAssets\Dto\BookAssetDto|null {
    ...
    /** @return array<string,mixed>|\BlackCat\Database\Packages\BookAssets\Dto\BookAssetDto|null */
    public function getById(int $id, bool $asDto = false): array|\BlackCat\Database\Packages\BookAssets\Dto\BookAssetDto|null {
    ...
    /** @return array<string,mixed>|\BlackCat\Database\Packages\BookAssets\Dto\BookAssetDto|null */
    public function getByIdForTenant(int|string $id, int|string $tenantId, bool $asDto = false): array|\BlackCat\Database\Packages\BookAssets\Dto\BookAssetDto|null {
