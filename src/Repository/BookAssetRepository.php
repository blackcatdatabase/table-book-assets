...
use BlackCat\Database\Packages\BookAssets\Repository\BookAssetRepositoryInterface;
...
class BookAssetRepository implements BookAssetRepositoryInterface, RepoContract, KeysetRepoContract
{
use OrderByTools, PkTools, RepositoryHelpers;

    /** @var mixed literal token for upsert keys (array or empty). */
    private mixed $tokenUpsertKeys = [ 'tenant_id', 'book_id', 'asset_type' ];
    ...
    public function upsertMany(array $rows): int {
        ...
        if ($helperKeys && class_exists(\BlackCat\Database\Support\BulkUpsertHelper::class)) {
          $bulk = new \BlackCat\Database\Support\BulkUpsertHelper($this->db, \BlackCat\Database\Packages\BookAssets\Definitions::class);
          $bulk->upsertMany($rows, $helperKeys, [ 'filename', 'mime_type', 'size_bytes', 'storage_path', 'content_hash', 'download_filename', 'is_encrypted', 'encryption_algo', 'encryption_key_enc', 'encryption_iv', 'encryption_tag', 'encryption_aad', 'encryption_meta', 'key_version', 'key_id' ]);
          return count($rows);
        }
    ...
    public function getByTenantIdAndBookIdAndAssetType(int $tenantId, int $bookId, string $assetType, bool $asDto = false): array|\BlackCat\Database\Packages\BookAssets\Dto\BookAssetDto|null {
        return $this->getByUnique([ 'tenant_id' => $tenantId, 'book_id' => $bookId, 'asset_type' => $assetType ], $asDto);
    }
    ...
    public function getIdByTenantIdAndBookIdAndAssetType(int $tenantId, int $bookId, string $assetType) {
        $row = $this->getByTenantIdAndBookIdAndAssetType($tenantId, $bookId, $assetType, false);
