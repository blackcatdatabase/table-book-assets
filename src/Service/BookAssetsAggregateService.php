...
namespace BlackCat\Database\Packages\BookAssets\Service;

use BlackCat\Core\Database;
use BlackCat\Database\Packages\BookAssets\Dto\BookAssetDto;
use BlackCat\Database\Packages\BookAssets\Mapper\BookAssetDtoMapper;
 ...
 */
final class BookAssetsAggregateService
{
    use ServiceHelpers;
    ...
    public function __construct(
        private Database $db, private BookAssetRepository $bookAssetRepo
    ) {
        // Repository autowiring (generated)
        $this->repositories = [
  'book_assets' => $this->bookAssetRepo
];
    }
    ...
    protected function withRowLock(callable $locker, int|string|array $id, callable $fetch, string $mode = 'wait'): mixed
    {
        ...
        return $this->txn(function () use ($locker, $id, $fetch, $mode) {
            ...
            if (!$row) {
                ...
                throw new \BlackCat\Database\Packages\BookAssets\ModuleException("Record {$idStr} not found for locking.");
            }
